...
import { urlFor, getPostSlug, BlogPost } from '../lib/sanity';
...
export default function BlogCard({ post, variant = 'default' }: BlogCardProps) {
  ...
  if (variant === 'compact') {
    return (
      <Link to={`/blog/${postSlug}`} className="group block">
        <div className="glass-card glass-card-hover rounded-2xl overflow-hidden">
          ...
          <div className="p-4">
            ...
            <p className="text-muted text-[10px] mt-2 font-body">{date}</p>
          </div>
        </div>
      </Link>
    );
    ...
    <Link to={`/blog/${postSlug}`} className="group block">
      <div className="glass-card glass-card-hover rounded-2xl overflow-hidden">
        ...
        <div className="p-5">
          ...
          <p className="text-muted text-[10px] mt-3 font-body">{date}</p>
        </div>
      </div>
    </Link>
  );
