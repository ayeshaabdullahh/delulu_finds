        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {features.map((feature) => (
            <div key={feature.title} className="glass-card glass-card-hover rounded-2xl p-6 sm:p-8 text-center">
              <div className="w-12 h-12 rounded-full bg-blush-100/60 flex items-center justify-center mx-auto mb-4">
                <feature.icon size={20} className="text-blush-300" />
              </div>
              <h3 className="font-display text-sm sm:text-base font-medium text-charcoal mb-1">{feature.title}</h3>
              <p className="text-gray-400 text-xs sm:text-sm font-body">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
