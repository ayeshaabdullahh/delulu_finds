    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled ? 'glass-nav shadow-lg' : 'bg-dark'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            >
              {mobileOpen ? <X size={22} className="text-mauve" /> : <Menu size={22} className="text-white" />}
            </button>
            ...
            <div className="hidden lg:flex items-center gap-8">
              ...
              <div className="relative" ref={categoriesRef}>
                <button
                  ...
                  className="text-sm font-semibold text-white/80 hover:text-white transition-colors relative group flex items-center gap-1"
                >
                  Categories
                  <ChevronDown size={14} className={`transition-transform ${categoriesOpen ? 'rotate-180' : ''}`} />
                  <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-mauve transition-all duration-300 group-hover:w-full rounded-full" />
                </button>
                {categoriesOpen && (
              ...
              <a
                href="#newsletter"
                className="bg-mauve hover:bg-mauve-light text-white text-sm font-semibold px-5 py-2.5 rounded-full transition-all hover:scale-105"
              >
                Newsletter
              </a>
            </div>
              ...
              <form onSubmit={handleSearch} className="hidden sm:flex items-center">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 rounded-full bg-white/10 border border-white/10 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-mauve/50 focus:ring-2 focus:ring-mauve/20 transition-all w-48 lg:w-64"
                  />
                </div>
              </form>
            ...
            <form onSubmit={handleSearch} className="max-w-xl mx-auto">
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search finds..."
                  className="w-full pl-9 pr-4 py-3 rounded-full bg-white/10 border border-white/10 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-mauve/50 focus:ring-2 focus:ring-mauve/20 transition-all"
                  autoFocus
                />
              </div>
            </form>
        ...
        >
          <div className="bg-dark px-4 pb-6 pt-2 border-t border-mauve/10">
            {/* Mobile categories */}
            <div className="mb-4">
              <p className="text-[10px] tracking-[0.2em] uppercase text-mauve font-bold mb-2 px-2">Categories</p>
            ...
            {links.map((link) => (
              <Link
                key={link.label}
                to={link.to}
                className="block py-3 text-sm font-semibold text-white/70 hover:text-white transition-colors border-b border-white/5 last:border-0"
                onClick={() => setMobileOpen(false)}
              >
                {link.label}
              </Link>
            ...
            <a
              href="#newsletter"
              className="block mt-4 bg-mauve text-white text-center text-sm font-semibold px-5 py-3 rounded-full"
              onClick={() => setMobileOpen(false)}
            >
              Newsletter
            </a>
      ...
      <div className="bottom-nav lg:hidden">
        <div className="flex items-center justify-around max-w-md mx-auto">
          <Link to="/" className="flex flex-col items-center gap-1 text-mauve">
          ...
          <a
            ...
            className="flex flex-col items-center gap-1 text-white/50 hover:text-mauve transition-colors"
          >
